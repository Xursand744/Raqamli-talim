...
const filters = [
  ...
  { name: "O'qish davomiyligi", items: [
    { name: "3 yil", count: 1 },
    { name: "4 yil", count: 2 },
    { name: "4 yildan ko'p", count: 0 },
  ]},
  { name: "O'qish formati", items: [
    { name: "Onlayn", count: 1 },
    { name: "Oflayn", count: 2 },
    { name: "Gibrid", count: 0 },
  ]},
...
export default function UniversityList() {
  ...
  const filteredUniversities = useMemo(() => {
    return universities.filter(university => {
      ...
      const matchesSpecialty = selectedSpecialty === '' || university.specialty === selectedSpecialty;
  ...
  return (
    <div className="flex mt-[100px] container bg-gray-50">
      {/* Sidebar */}
      <div className={`fixed  inset-y-25 left-0 z-30 w-64 bg-white transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out md:relative md:translate-x-0`}>
        <div className="p-4">
      ...
      <div className="flex-1 p-4 md:p-6">
        ...
        <button className="mb-4 p-2 bg-blue-500 text-white rounded-lg md:hidden" onClick={toggleSidebar}>
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <h2 className="mb-4 text-lg font-semibold">Universitetlar soni: {filteredUniversities.length}</h2>

        {/* University grid */}
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filteredUniversities.map((university) => (
            <div key={university.id} className="overflow-hidden max-w-[282px] w-full bg-white border rounded-lg shadow-sm">
              ...
              <div className="p-4">
                ...
                <div className="flex flex-wrap gap-1 mb-2">
                  {university.programs.map((program) => (
                    <span key={program} className="px-2 py-1 text-xs bg-gray-100 rounded">
                      {program}
                    </span>
                  ))}
                </div>
                <div className="text-xs text-gray-500">{university.type}</div>
                <div className="mt-2 text-sm text-blue-500">{university.phone}</div>
              </div>
            </div>
          ))}
        </div>
