...
const NewsCard = ({ id, date, title, description, image, tags }) => {
  const navigate = useNavigate();

  return (
    <motion.div
      onClick={() => navigate(`/news/${id}`)}
      className="cursor-pointer flex flex-col sm:flex-row bg-white rounded-xl shadow-sm overflow-hidden hover:shadow-xl transition-all duration-300"
